...
# 2.Working directories--------
  ...
  dir$Pre_monthly <- paste(dir$root,"/Data/Raw_Data/Presciption_monthly",sep="")
...
# 3.Download data from "http://hydrology.princeton.edu/data/pgf/v3/0.25deg/daily/" -----
#Monthly data
  url <- "http://hydrology.princeton.edu/data/pgf/v2/0.5deg/monthly/prcp_monthly_1901-2012.nc"
  destfile <- paste(dir$Pre_monthly,"/prcp_monthly_1901-2012.nc",sep="")
  download.file(url, destfile=destfile, method="curl", quiet = FALSE, mode = "w")

# 4.Read Data and Manipulation--------
  flist <- list.files(dir$Pre_monthly, full.names=T)
  Pre_mon <- brick(flist[1])
...
## 6.1.Function to shift a column upward-------
  shift <- function(x, n){
    c(x[-(seq(n))], rep(NA, n))
  }

## 6.2.Function to calculate H--------
...
# 7.Main Code-------
  ...
  for (j in 1:12){
    ...
    Pre_month<- Pre_mon_cn[][,month]
    for (i in 1:dim(Pre_month[])[1]){
      tmp <- Pre_month[][i,]
      if (sum(!is.na(tmp))>=2){
  ...
  Pre_mon_cn[][,seq(1,12)] <- H_list

# 8.Draw The map ----- 
  png(paste(dir$output,"/Pre_H.png",sep=""), width=1000*4, height = 1000*3, res=80*4)
  
  for (i in 1:12){
    num <- paste("p",i,sep="")
    plot_data <- as.data.frame(Pre_mon_cn[[i]],xy=TRUE)
